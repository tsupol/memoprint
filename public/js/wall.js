...
jQuery(document).ready(function ($){
    ...
    function flipCard(targetId, hIdx, delay, isProfile, isSwap) {
        ...
        if(isProfile) {
            var userId = 0;
            if($.isNumeric(isProfile)) {
                console.log('isProfile', isProfile);
                userId = isProfile;
            } else {
                userId = pics[hIdx].user_id;
            }
    ...
    $( document ).keypress(function( event ) {
        if ( event.which == 100 ) {
            ...
            $cards.children().each(function () {
                var me = $(this);
                var randId = getRandomInt(0, maxId);
                flipCard(me.index(), 0, i*200, users[randId]);
            });
        }
    });
